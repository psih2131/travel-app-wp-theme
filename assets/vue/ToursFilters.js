/**
 * Компонент: фильтры рубрик и список туров (TourCard).
 * Вся разметка здесь.
 */
(function () {
    'use strict';
            ...
            <div class="travel-tours-filters">
              ...
              <div class="tours-hero-sec__tegs-wrapper"><h2 class="tours-hero-sec__tegs-title">Рубрики</h2>
                <div class="tours-hero-sec__tegs-row">
                  ...
                  <button type="button" v-for="r in rubrics" :key="r.id" class="tours-hero-sec__teg tour-teg" :class="{ active: activeRubricId === r.id }" @click="$emit('set-rubric', r.id)">{{ r.name }} <span class="tour-teg__counter">{{ r.count }}</span></button>
                </div></div>
